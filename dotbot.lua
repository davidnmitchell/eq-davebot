...
local function HasDot(castable, id)
	return mq.TLO.Spawn(id).Buff(castable.Effect)() ~= nil
end
...
local function do_dots()
	...
	for pct, spell_key in pairs(Config.Dot.AtTargetHpPcts()) do
		...
		if res.gem < 1 then
			...
			if group_target_id and not target.IsInGroup(group_target_id) and hps_are_in_range(group_target_id, pct) and not HasDot(castable, group_target_id) then
				CastDotOn(castable.Name, res.gem, group_target_id, i)
			end
		end
		i = i + 1
	end
